...
use oxc_ast::ast::{Statement, TSInterfaceDeclaration, TSTypeAliasDeclaration};
...
pub fn flatten_ts(content: &str, type_name: &str) -> Result<()> {
    ...
    let ast_semantic = semantic.semantic;
    ...
    match target_type {
        DeclRef::Interface(decl) => {
            let _result_interface = interface::flatten_type(&decl, &ast_semantic, &env, &allocator);
        }
        DeclRef::TypeAlias(decl) => {
            let _result_type = type_alias::flatten_type(&decl, &ast_semantic, &env, &allocator);
        }
    };
